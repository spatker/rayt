...
impl Camera {
    ...
    fn capture_pixel(&self, h: f32, w: f32, resolution: &Resolution,  scene: &Scene) -> Color {
        let pos_on_plane = Vec3{
            x: (h + 0.5 - (resolution.width/2) as f32) / (resolution.width/2) as f32,
            y: (w + 0.5 - (resolution.height/2) as f32) / (resolution.height/2) as f32, 
            z: 0.0
        };
      
        let plane_intersection = self.plane_pos + pos_on_plane.x * self.rigth + pos_on_plane.y * self.up;
